...
/* V6.4 workspace visual system */
.cc-title-row{display:flex;justify-content:space-between;align-items:flex-end;gap:1rem;margin:.2rem 0 .85rem}.cc-title-row h1{margin:0!important;font-size:2.15rem!important}.cc-title-row p{margin:.25rem 0 0;color:#64748B;font-size:.98rem}.cc-date{text-align:right;color:#64748B;font-size:.78rem;white-space:nowrap}
.kpi-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.75rem;margin:.55rem 0 1rem}.kpi-card{background:#fff;border:1px solid #DCE6EC;border-radius:13px;padding:.8rem .9rem;box-shadow:0 3px 12px rgba(15,39,68,.04)}.kpi-label{font-size:.76rem;color:#64748B;font-weight:750}.kpi-value{font-size:1.65rem;color:#102A43;font-weight:850;line-height:1.1;margin:.18rem 0}.kpi-note{font-size:.72rem;color:#7A8D9C}.kpi-alert .kpi-value{color:#B42318}.kpi-good .kpi-value{color:#16794B}.kpi-blue .kpi-value{color:#175CD3}
.ai-panel{background:linear-gradient(135deg,#102A43,#123D61);border:1px solid #244E70;border-radius:16px;padding:1rem 1.15rem;color:#F8FBFF;margin:.55rem 0 1rem;box-shadow:0 8px 24px rgba(15,39,68,.12)}.ai-panel .section-eyebrow{color:#67E8F9!important}.ai-panel strong{font-size:1.05rem}.ai-panel p{color:#C8D8E6;margin:.35rem 0 0;font-size:.9rem;line-height:1.45}.ai-chips{display:flex;flex-wrap:wrap;gap:.4rem;margin-top:.7rem}.ai-chip{border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);border-radius:999px;padding:.3rem .55rem;font-size:.72rem;color:#EAF5FA}
.patient-banner{display:flex;justify-content:space-between;gap:1rem;align-items:center;background:linear-gradient(135deg,#FFFFFF,#F2FAFA);border:1px solid #D6E7E8;border-radius:16px;padding:1rem 1.1rem;margin:.35rem 0 .9rem;box-shadow:0 4px 16px rgba(15,39,68,.045)}.patient-name{font-size:1.42rem;font-weight:850;color:#102A43}.patient-meta{font-size:.84rem;color:#64748B;margin-top:.18rem}.patient-tags{display:flex;gap:.4rem;flex-wrap:wrap;justify-content:flex-end}.patient-tag{padding:.32rem .58rem;border-radius:999px;background:#E8F7F5;color:#075F68;font-size:.73rem;font-weight:800}.patient-tag.priority{background:#EEF4FF;color:#175CD3}.subtle-note{color:#64748B;font-size:.84rem;line-height:1.45}
.research-hero{background:linear-gradient(135deg,#F2F7FF 0%,#F2FBFA 55%,#FFFFFF 100%);border:1px solid #D7E4F0;border-radius:18px;padding:1.2rem 1.3rem;margin:.35rem 0 1rem}.research-hero h1{font-size:2.1rem!important;margin:.15rem 0!important}.research-hero p{color:#5E7184;max-width:980px;margin:.3rem 0 0}.research-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.75rem;margin:.7rem 0 1rem}.research-card{background:#fff;border:1px solid #DCE6EC;border-radius:14px;padding:.9rem 1rem}.research-card b{display:block;color:#102A43;font-size:.96rem}.research-card span{display:block;color:#64748B;font-size:.8rem;line-height:1.4;margin-top:.22rem}.research-card .num{font-size:1.5rem;color:#175CD3;font-weight:850;margin-bottom:.15rem}
...
if menu.startswith('1 ·'):
    ...
    st.markdown('<div class="cc-title-row"><div><div class="section-eyebrow">CONNECTED CARE · MONITOR</div><h1>Clinical Command Center</h1><p>Real-time prioritization, patient surveillance and human action across the RPM population.</p></div><div class="cc-date">Clinical operations workspace<br><b>Human-in-the-loop</b></div></div>',unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-eyebrow">CONNECTED CARE · MONITOR</div><h1>Patient 360</h1><div class="subtle-note">A longitudinal clinical workspace for measurements, devices, symptoms, results, documents and interventions.</div>',unsafe_allow_html=True)
    pid=patient_picker('Find patient','trends_patient'); p=PATIENTS[pid]
    e=latest_for(pid); _p360_priority='AWAITING DATA' if not e else assess(e)[0]
    st.markdown(f'''<div class="patient-banner"><div><div class="patient-name">{p['name']}</div><div class="patient-meta">{p['mrn']} · {p['care_plan']} · Assigned to {p['clinician']}</div></div><div class="patient-tags"><span class="patient-tag">{p.get('program_duration','Ongoing / clinician-defined')}</span><span class="patient-tag priority">{_p360_priority}</span></div></div>''',unsafe_allow_html=True)
    st.caption('Trend convention: teal dashed = configured threshold · purple = manual entry · red = actual out-of-threshold reading.')
    if not e:
...
KEY PRINCIPLE: one enterprise patient ecosystem, multiple role- and lifecycle-specific views. Operational MRN ≠ Research Participant ID.''')
    ...
    st.markdown('''<div class="research-hero"><div class="section-eyebrow">RESEARCH & INSIGHTS · LEARN</div><h1>Research Analytics Center</h1><p>Turn governed, de-identified longitudinal RPM data into reproducible cohorts, descriptive evidence and questions for prospective validation.</p></div>''',unsafe_allow_html=True)
    ...
    cohort_n=df['Cohort'].nunique()
    st.markdown(f'''<div class="kpi-grid"><div class="kpi-card kpi-blue"><div class="kpi-label">PARTICIPANTS</div><div class="kpi-value">{parts:,}</div><div class="kpi-note">De-identified research IDs</div></div><div class="kpi-card"><div class="kpi-label">COHORTS</div><div class="kpi-value">{cohort_n:,}</div><div class="kpi-note">Purpose-defined populations</div></div><div class="kpi-card"><div class="kpi-label">OBSERVATIONS</div><div class="kpi-value">{obs:,}</div><div class="kpi-note">Longitudinal records</div></div><div class="kpi-card kpi-alert"><div class="kpi-label">FLAGGED</div><div class="kpi-value">{alerts:,}</div><div class="kpi-note">Observations requiring context</div></div></div>''',unsafe_allow_html=True)
    st.markdown(f'''<div class="research-grid"><div class="research-card"><div class="num">{spi:,}</div><b>Spirometry sessions</b><span>FEV₁, FVC, ratio, PEF, personal-baseline change and session quality.</span></div><div class="research-card"><div class="num">{ecg:,}</div><b>6L ECG recordings</b><span>Device-reported classifications, heart rate, symptoms and downstream human action.</span></div><div class="research-card"><div class="num">{cohort_n}</div><b>Reproducible cohorts</b><span>Operational care path remains distinct from research cohort and analytical subgroup.</span></div></div>''',unsafe_allow_html=True)
    st.info('Research identity is de-identified by default. Results are descriptive/exploratory synthetic outputs for hypothesis generation and operational learning—not diagnosis, treatment or causal claims.')
